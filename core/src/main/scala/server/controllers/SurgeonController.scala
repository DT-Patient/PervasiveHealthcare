  ...
  def handleAction(): Behavior[CQRSAction] =
    Behaviors.receiveMessage {
      case InsertMedicalRecord(medicalRecord, replyTo) =>
        val res =
          Repository.surgeonRepository.insertMedicalRecord(medicalRecord)
      ...
      case UpdateMedicalRecord(medicalRecordID, medicalRecord, replyTo) =>
        val res = Repository.surgeonRepository.updateMedicalRecord(
          medicalRecordID,
          medicalRecord
        )
        ...
        Behaviors.same
      case GetAllMedicalRecordsForDoctor(doctorID, replyTo) =>
        val res = RMUtility.getAllMedicalRecordsForDoctor(doctorID)
        replyTo ! res
        Behaviors.same
    }
}
