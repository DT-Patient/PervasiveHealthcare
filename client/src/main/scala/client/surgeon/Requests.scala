...
object Requests {
   ...
   */
  def insertMedicalRecordRequest(token: String, medicalRecord: MedicalRecord)(
   ...
   * @return http response of patient
   */
  def updateMedicalRecordRequest(token: String, medicalRecord: MedicalRecord)(
    implicit system: ClassicActorSystemProvider
  ): Future[HttpResponse] = {
    Http().singleRequest(
      HttpRequest(
        ...
        uri =
          s"http://127.0.0.1:8080/api/medicalrecords/${medicalRecord.medicalRecordID.value}",
        headers = List(APITokenHeader(token)),
        entity = HttpEntity(
          ContentTypes.`application/json`,
          s"${medicalRecord.toJson}"
   ...
   * @return http response of patient
   */
  def allMedicalRecordsRequest(token: String, doctorID: DoctorID)(implicit
