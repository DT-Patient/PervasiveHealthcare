...
class SurgeonActor(doctorID: DoctorID, token: String)
  extends Actor
  with ActorLogging {
  ...
  private lazy val onAttendResponseInsertMedicalRecordMessageBehaviour
    : Receive = {
  ...
  private lazy val onAttendResponseUpdateMedicalRecordMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.OK, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        println(body.utf8String)
  ...
  private lazy val onAttendResponseAllMedicalRecordsMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.OK, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        val medicalRecords: Seq[MedicalRecord] =
          JsonParser(body.utf8String).convertTo[Seq[MedicalRecord]]
