...
class GeneralPractitionerActor(doctorID: DoctorID, token: String)
  ...
  private lazy val onInteractionBehaviour: Receive = {
    ...
    case InsertGeneralPractitionerInfoMessage(generalPractitionerInfo) =>
      insertGeneralPractitionerInfoRequest(token, generalPractitionerInfo)
        .pipeTo(self)
    ...
    case UpdateGeneralPractitionerInfoMessage(generalPractitionerInfo) =>
      updateGeneralPractitionerInfoRequest(
        token,
        generalPractitionerInfo.patientID,
  ...
  private lazy val onAttendResponseGeneralPractitionerInfoMessageBehaviour
    : Receive = {
  ...
  private lazy val onAttendResponseInsertGeneralPractitionerInfoMessageBehaviour
    : Receive = {
  ...
  private lazy val onAttendResponseUpdateGeneralPractitionerInfoMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.OK, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        println(body.utf8String)
  ...
  private lazy val onAttendResponseCardiologyPredictionMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.OK, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        val cardiologyPrediction: CardiologyPrediction =
          JsonParser(body.utf8String).convertTo[CardiologyPrediction]
