...
import client.cardiologist.Message.{
  AllCardiologyVisitsMessage,
  InsertCardiologyVisitMessage
}
import client.cardiologist.Requests.{
  allCardiologyVisitsRequest,
  insertCardiologyVisitRequest
}
...
class CardiologistActor(doctorID: DoctorID, token: String)
  extends Actor
  with ActorLogging {
  ...
  implicit val executionContext: ExecutionContextExecutor =
    Client.system.classicSystem.dispatcher
  ...
  private lazy val onAttendResponseInsertCardiologyVisitMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.Created, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        println(body.utf8String)
        val cardiologyVisit: CardiologyVisit =
          JsonParser(body.utf8String).convertTo[CardiologyVisit]
  ...
  private lazy val onAttendResponseAllCardiologyVisitsMessageBehaviour
    : Receive = {
    case HttpResponse(StatusCodes.OK, _, entity, _) =>
      entity.dataBytes.runFold(ByteString(""))(_ ++ _).foreach { body =>
        val cardiologyVisits: Seq[CardiologyVisit] =
          JsonParser(body.utf8String).convertTo[Seq[CardiologyVisit]]
      }
      this.context become onInteractionBehaviour
    case resp @ HttpResponse(code, _, _, _) =>
      println("Error: " + code.value)
